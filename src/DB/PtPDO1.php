...
//            print_r( $data );
    ...
    {
        if (Context::get(self::$isInTransactionKey)) { //嵌套事务
         //   throw new DBException('不支持事务嵌套', 200100);
        }
        $this->realGetConn();
        $this->_pdo->beginTransaction();
        Context::set(self::$isInTransactionKey, 1);
        $this->_pdo->_transaction_id = Tool::getRandChar(5);
        print_r('beginTransaction事务ID:' . $this->_pdo->_transaction_id . PHP_EOL);
        Context::set( $this->_pdo);
    ...
    public function realGetConn()
    {
        if (Context::get(self::$isInTransactionKey)) {
            $this->_pdo = Context::get(self::$TransactionPDOKey);
            print_r('realGetConn事务ID:' . $this->_pdo->_transaction_id . PHP_EOL);
        } else {
     ...
     * @return bool
     */
    public function release($connection = null,$isCommit=false)
    {
