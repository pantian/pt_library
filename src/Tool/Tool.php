...
class Tool
{
     ...
     * @param $arr
     */
    public static function setIndex($key, array &$arr)
    {
     ...
     * @return false|mixed
     */
    public static function getNext(array &$arr)
    {
        return next($arr) !== false ? current($arr) : false;
    }
...
//		$IPLocation=IpLocation::getInstance();
        $IPLocation = IPQuery::instance();
        $info = $IPLocation->query($ip);
        $city = Tool::getArrVal('pos', $info);
        return $city;
...
//		return \swoole_serialize::unpack( $data );
     ...
     * @return array  转换后的树
     */
    static function ListToTree($dataArr, $rootId = 0, $pkName = 'cc_id', $pIdName = 'parent_id', $childName = 'children')
    {
        $tree = [];
        if (is_array($dataArr)) {
     ...
     *
     * @param array $array
     * @param array $need_keys
     * @return bool
     */
    static function verifyArray(array $array,array $need_keys){
        $orgin_count = count($need_keys);
        if ($orgin_count == 0) {
            return true;
        }
        $keys = array_keys($array);
        $intersectList = array_intersect($keys, $need_keys);
        if (count($intersectList) === $orgin_count) {
            return true;
        } else {
            return false;
        }
